...
namespace TrackAndTrace_API.Controllers
{
    ...
    {
        private readonly IMaterialRepository _interface;
        private readonly ApplicationDbContext _context;
        public materialController(IMaterialRepository Interface, ApplicationDbContext context)
        {
        ...
        [HttpPost]
        public async Task<ActionResult> Add([FromBody] MaterialDto model)
        {
        ...
        [HttpGet("brand_mapping/{id}")]
        public async Task<ActionResult> GetBrandMappingById(int id)
        {
            APIResponseDTO aPIResponseDTO = new APIResponseDTO();
            ...
            if (token != null)
            {
                aPIResponseDTO = await _interface.GetBrandMappingById(id);
            }
            else
            {
                return Unauthorized();
        ...
        [HttpGet("material_dropdown")]
        public async Task<ActionResult> GetMaterialDropdown()
        {
            APIResponseDTO aPIResponseDTO = new APIResponseDTO();
            ...
            if (token != null)
            {
                aPIResponseDTO = await _interface.GetMaterialDropdown(token);
            }
            else
            {
                return Unauthorized();
