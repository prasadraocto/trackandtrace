...
namespace TrackAndTrace_API.Repository
{
    public class ShiftRepository: IShiftRepository
    {
        ...
        public ShiftRepository(ApplicationDbContext context, IMapper mapper, IConfiguration configuration)
        {
            _context = context;
            _mapper = mapper;
        ...
        public async Task<APIResponseDTO> Add(ShiftDto model, ExtractTokenDto token)
        {
            APIResponseDTO aPIResponseDTO = new APIResponseDTO();

            try
            {
                var nameExists = await _context.Shift.AnyAsync(x => x.name.ToLower() == model.name.ToLower() && x.id != model.id && x.company_id == token.CompanyId);
                if (nameExists)
                {
                    aPIResponseDTO.message = "Shift Name already exists";
                    return aPIResponseDTO;
                }

                var codeExists = await _context.Shift.AnyAsync(x => x.code.ToLower() == model.code.ToLower() && x.id != model.id && x.company_id == token.CompanyId);
                if (codeExists)
                {
                    aPIResponseDTO.message = "Shift Code already exists";
                    return aPIResponseDTO;
                }

                var dmo = _mapper.Map<Shift>(model);

                if (dmo.id > 0)
                {
                    var existingShift = await _context.Shift.AsNoTracking().FirstOrDefaultAsync(x => x.id == dmo.id);

                    if (existingShift == null)
                    {
                        aPIResponseDTO.message = "Shift details not found";
                        return aPIResponseDTO;
                    }

                    dmo.company_id = token.CompanyId;
                    dmo.created_by = existingShift.created_by;
                    dmo.created_date = existingShift.created_date;
                    dmo.updated_by = token.UserId;
                    dmo.updated_date = DateTime.Now;
                    _context.Shift.Update(dmo);
                ...
                else
                {
                    ...
                    await _context.Shift.AddAsync(dmo);
                    await _context.SaveChangesAsync();
                }

                aPIResponseDTO.success = true;
                aPIResponseDTO.message = "Shift saved successfully";
                return aPIResponseDTO;
            }
            catch (Exception ex)
            {
        ...
        public async Task<APIResponseDTO> GetList(CommonRequestDto request, ExtractTokenDto token)
        {
            ...
            try
            {
                ...
                {
                    using (var command = new SqlCommand("get_shift_list", connection))
                    {
        ...
        public async Task<APIResponseDTO> Delete(string ids, ExtractTokenDto token)
        {
            ...
            try
            {
                ...
                var data = await _context.Shift.Where(x => x.delete_flag == false && idsToDelete.Contains(x.id) && x.company_id == token.CompanyId).ToListAsync();
                ...
                if (data.Any())
                {
                    ...
                    _context.Shift.UpdateRange(data);
                    await _context.SaveChangesAsync();

                    aPIResponseDTO.success = true;
                    aPIResponseDTO.message = "Shift deleted successfully.";
                }
                else
                {
        ...
        public async Task<APIResponseDTO> ActiveInactive(int id, ExtractTokenDto token)
        {
            ...
            try
            {
                var data = await _context.Shift.Where(x => x.id == id).FirstOrDefaultAsync();
                ...
                if (data != null)
                {
                    ...
                    _context.Shift.Update(data);
                    await _context.SaveChangesAsync();

                    aPIResponseDTO.success = true;
                    aPIResponseDTO.message = "Shift " + (data.active_flag == true ? "Activated" : "Inactivated") + " successfully.";
                }
                else
                {
