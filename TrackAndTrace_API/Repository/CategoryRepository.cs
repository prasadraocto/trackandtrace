...
namespace TrackAndTrace_API.Repository
{
    public class CategoryRepository: ICategoryRepository
    {
        ...
        public CategoryRepository(ApplicationDbContext context, IMapper mapper, IConfiguration configuration)
        {
            _context = context;
            _mapper = mapper;
        ...
        public async Task<APIResponseDTO> Add(CategoryDto model, ExtractTokenDto token)
        {
            APIResponseDTO aPIResponseDTO = new APIResponseDTO();

            try
            {
                var nameExists = await _context.Category.AnyAsync(x => x.name.ToLower() == model.name.ToLower() && x.id != model.id && x.company_id == token.CompanyId);
                if (nameExists)
                {
                    aPIResponseDTO.message = "Category Name already exists";
                    return aPIResponseDTO;
                }

                var codeExists = await _context.Category.AnyAsync(x => x.code.ToLower() == model.code.ToLower() && x.id != model.id && x.company_id == token.CompanyId);
                if (codeExists)
                {
                    aPIResponseDTO.message = "Category Code already exists";
                    return aPIResponseDTO;
                }

                var dmo = _mapper.Map<Category>(model);

                if (dmo.id > 0)
                {
                    var existingCategory = await _context.Category.AsNoTracking().FirstOrDefaultAsync(x => x.id == dmo.id);

                    if (existingCategory == null)
                    {
                        aPIResponseDTO.message = "Category details not found";
                        return aPIResponseDTO;
                    }

                    dmo.company_id = token.CompanyId;
                    dmo.created_by = existingCategory.created_by;
                    dmo.created_date = existingCategory.created_date;
                    dmo.updated_by = token.UserId;
                    dmo.updated_date = DateTime.Now;
                    _context.Category.Update(dmo);
                ...
                else
                {
                    ...
                    await _context.Category.AddAsync(dmo);
                    await _context.SaveChangesAsync();
                }

                aPIResponseDTO.success = true;
                aPIResponseDTO.message = "Category saved successfully";
                return aPIResponseDTO;
            }
            catch (Exception ex)
            {
        ...
        public async Task<APIResponseDTO> GetList(CommonRequestDto request, ExtractTokenDto token)
        {
            ...
            try
            {
                ...
                {
                    using (var command = new SqlCommand("get_category_list", connection))
                    {
        ...
        public async Task<APIResponseDTO> Delete(string ids, ExtractTokenDto token)
        {
            ...
            try
            {
                ...
                var data = await _context.Category.Where(x => x.delete_flag == false && idsToDelete.Contains(x.id) && x.company_id == token.CompanyId).ToListAsync();
                ...
                if (data.Any())
                {
                    ...
                    _context.Category.UpdateRange(data);
                    await _context.SaveChangesAsync();

                    aPIResponseDTO.success = true;
                    aPIResponseDTO.message = "Category deleted successfully.";
                }
                else
                {
        ...
        public async Task<APIResponseDTO> ActiveInactive(int id, ExtractTokenDto token)
        {
            ...
            try
            {
                var data = await _context.Category.Where(x => x.id == id).FirstOrDefaultAsync();
                ...
                if (data != null)
                {
                    ...
                    aPIResponseDTO.success = true;
                    aPIResponseDTO.message = "Category " + (data.active_flag == true ? "Activated" : "Inactivated") + " successfully.";
                }
                else
                {
