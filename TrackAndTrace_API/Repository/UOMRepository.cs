...
namespace TrackAndTrace_API.Repository
{
    public class UOMRepository: IUOMRepository
    {
        ...
        public UOMRepository(ApplicationDbContext context, IMapper mapper, IConfiguration configuration)
        {
            _context = context;
            _mapper = mapper;
        ...
        public async Task<APIResponseDTO> Add(UOMDto model, ExtractTokenDto token)
        {
            APIResponseDTO aPIResponseDTO = new APIResponseDTO();

            try
            {
                var nameExists = await _context.UOM.AnyAsync(x => x.name.ToLower() == model.name.ToLower() && x.id != model.id && x.company_id == token.CompanyId);
                if (nameExists)
                {
                    aPIResponseDTO.message = "UOM Name already exists";
                    return aPIResponseDTO;
                }

                var codeExists = await _context.UOM.AnyAsync(x => x.code.ToLower() == model.code.ToLower() && x.id != model.id && x.company_id == token.CompanyId);
                if (codeExists)
                {
                    aPIResponseDTO.message = "UOM Code already exists";
                    return aPIResponseDTO;
                }

                var dmo = _mapper.Map<UOM>(model);

                if (dmo.id > 0)
                {
                    var existingUOM = await _context.UOM.AsNoTracking().FirstOrDefaultAsync(x => x.id == dmo.id);

                    if (existingUOM == null)
                    {
                        aPIResponseDTO.message = "UOM details not found";
                        return aPIResponseDTO;
                    }

                    dmo.company_id = token.CompanyId;
                    dmo.created_by = existingUOM.created_by;
                    dmo.created_date = existingUOM.created_date;
                    dmo.updated_by = token.UserId;
                    dmo.updated_date = DateTime.Now;
                    _context.UOM.Update(dmo);
                ...
                else
                {
                    ...
                    await _context.UOM.AddAsync(dmo);
                    await _context.SaveChangesAsync();
                }

                aPIResponseDTO.success = true;
                aPIResponseDTO.message = "UOM saved successfully";
                return aPIResponseDTO;
            }
            catch (Exception ex)
            {
        ...
        public async Task<APIResponseDTO> GetList(CommonRequestDto request, ExtractTokenDto token)
        {
            ...
            try
            {
                ...
                {
                    using (var command = new SqlCommand("get_uom_list", connection))
                    {
        ...
        public async Task<APIResponseDTO> Delete(string ids, ExtractTokenDto token)
        {
            ...
            try
            {
                ...
                var data = await _context.UOM.Where(x => x.delete_flag == false && idsToDelete.Contains(x.id) && x.company_id == token.CompanyId).ToListAsync();
                ...
                if (data.Any())
                {
                    ...
                    _context.UOM.UpdateRange(data);
                    await _context.SaveChangesAsync();

                    aPIResponseDTO.success = true;
                    aPIResponseDTO.message = "UOM deleted successfully.";
                }
                else
                {
        ...
        public async Task<APIResponseDTO> ActiveInactive(int id, ExtractTokenDto token)
        {
            ...
            try
            {
                var data = await _context.UOM.Where(x => x.id == id).FirstOrDefaultAsync();
                ...
                if (data != null)
                {
                    ...
                    _context.UOM.Update(data);
                    await _context.SaveChangesAsync();

                    aPIResponseDTO.success = true;
                    aPIResponseDTO.message = "UOM " + (data.active_flag == true ? "Activated" : "Inactivated") + " successfully.";
                }
                else
                {
