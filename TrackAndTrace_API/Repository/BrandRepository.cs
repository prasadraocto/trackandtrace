...
namespace TrackAndTrace_API.Repository
{
    public class BrandRepository: IBrandRepository
    {
        ...
        public BrandRepository(ApplicationDbContext context, IMapper mapper, IConfiguration configuration)
        {
            _context = context;
            _mapper = mapper;
        ...
        public async Task<APIResponseDTO> Add(BrandDto model, ExtractTokenDto token)
        {
            APIResponseDTO aPIResponseDTO = new APIResponseDTO();

            try
            {
                var nameExists = await _context.Brand.AnyAsync(x => x.name.ToLower() == model.name.ToLower() && x.id != model.id && x.company_id == token.CompanyId);
                if (nameExists)
                {
                    aPIResponseDTO.message = "Brand Name already exists";
                    return aPIResponseDTO;
                }

                var codeExists = await _context.Brand.AnyAsync(x => x.code.ToLower() == model.code.ToLower() && x.id != model.id && x.company_id == token.CompanyId);
                if (codeExists)
                {
                    aPIResponseDTO.message = "Brand Code already exists";
                    return aPIResponseDTO;
                }

                var dmo = _mapper.Map<Brand>(model);

                if (dmo.id > 0)
                {
                    var existingBrand = await _context.Brand.AsNoTracking().FirstOrDefaultAsync(x => x.id == dmo.id);

                    if (existingBrand == null)
                    {
                        aPIResponseDTO.message = "Brand details not found";
                        return aPIResponseDTO;
                    }

                    dmo.company_id = token.CompanyId;
                    dmo.created_by = existingBrand.created_by;
                    dmo.created_date = existingBrand.created_date;
                    dmo.updated_by = token.UserId;
                    dmo.updated_date = DateTime.Now;
                    _context.Brand.Update(dmo);
                ...
                else
                {
                    ...
                    await _context.Brand.AddAsync(dmo);
                    await _context.SaveChangesAsync();
                }

                aPIResponseDTO.success = true;
                aPIResponseDTO.message = "Brand saved successfully";
                return aPIResponseDTO;
            }
            catch (Exception ex)
            {
        ...
        public async Task<APIResponseDTO> GetList(CommonRequestDto request, ExtractTokenDto token)
        {
            ...
            try
            {
                ...
                {
                    using (var command = new SqlCommand("get_brand_list", connection))
                    {
        ...
        public async Task<APIResponseDTO> Delete(string ids, ExtractTokenDto token)
        {
            ...
            try
            {
                ...
                var data = await _context.Brand.Where(x => x.delete_flag == false && idsToDelete.Contains(x.id) && x.company_id == token.CompanyId).ToListAsync();
                ...
                if (data.Any())
                {
                    ...
                    _context.Brand.UpdateRange(data);
                    await _context.SaveChangesAsync();

                    aPIResponseDTO.success = true;
                    aPIResponseDTO.message = "Brand deleted successfully.";
                }
                else
                {
        ...
        public async Task<APIResponseDTO> ActiveInactive(int id, ExtractTokenDto token)
        {
            ...
            try
            {
                var data = await _context.Brand.Where(x => x.id == id).FirstOrDefaultAsync();
                ...
                if (data != null)
                {
                    ...
                    aPIResponseDTO.success = true;
                    aPIResponseDTO.message = "Brand " + (data.active_flag == true ? "Activated" : "Inactivated") + " successfully.";
                }
                else
                {
