...
namespace TrackAndTrace_API.Repository
{
    public class SubcontractorRepository: ISubcontractorRepository
    {
        ...
        public SubcontractorRepository(ApplicationDbContext context, IMapper mapper, IConfiguration configuration)
        {
            _context = context;
            _mapper = mapper;
        ...
        public async Task<APIResponseDTO> Add(SubcontractorDto model, ExtractTokenDto token)
        {
            APIResponseDTO aPIResponseDTO = new APIResponseDTO();

            try
            {
                var nameExists = await _context.Subcontractor.AnyAsync(x => x.name.ToLower() == model.name.ToLower() && x.id != model.id && x.company_id == token.CompanyId);
                if (nameExists)
                {
                    aPIResponseDTO.message = "Subcontractor Name already exists";
                    return aPIResponseDTO;
                }

                var codeExists = await _context.Subcontractor.AnyAsync(x => x.code.ToLower() == model.code.ToLower() && x.id != model.id && x.company_id == token.CompanyId);
                if (codeExists)
                {
                    aPIResponseDTO.message = "Subcontractor Code already exists";
                    return aPIResponseDTO;
                }

                var dmo = _mapper.Map<Subcontractor>(model);

                if (dmo.id > 0)
                {
                    var existingSubcontractor = await _context.Subcontractor.AsNoTracking().FirstOrDefaultAsync(x => x.id == dmo.id);

                    if (existingSubcontractor == null)
                    {
                        aPIResponseDTO.message = "Subcontractor details not found";
                        return aPIResponseDTO;
                    }

                    dmo.company_id = token.CompanyId;
                    dmo.created_by = existingSubcontractor.created_by;
                    dmo.created_date = existingSubcontractor.created_date;
                    dmo.updated_by = token.UserId;
                    dmo.updated_date = DateTime.Now;
                    _context.Subcontractor.Update(dmo);
                ...
                else
                {
                    ...
                    await _context.Subcontractor.AddAsync(dmo);
                    await _context.SaveChangesAsync();
                }

                aPIResponseDTO.success = true;
                aPIResponseDTO.message = "Subcontractor saved successfully";
                return aPIResponseDTO;
            }
            catch (Exception ex)
            {
        ...
        public async Task<APIResponseDTO> GetList(CommonRequestDto request, ExtractTokenDto token)
        {
            ...
            try
            {
                ...
                {
                    using (var command = new SqlCommand("get_subcontractor_list", connection))
                    {
                        ...
                        using (var reader = await command.ExecuteReaderAsync())
                        {
                            while (await reader.ReadAsync())
                            {
                                var data = new
                                {
                                    ...
                                    labour_type_name = reader.GetString(reader.GetOrdinal("labour_type_name")),
        ...
        public async Task<APIResponseDTO> Delete(string ids, ExtractTokenDto token)
        {
            ...
            try
            {
                ...
                var data = await _context.Subcontractor.Where(x => x.delete_flag == false && idsToDelete.Contains(x.id) && x.company_id == token.CompanyId).ToListAsync();
                ...
                if (data.Any())
                {
                    ...
                    _context.Subcontractor.UpdateRange(data);
                    await _context.SaveChangesAsync();

                    aPIResponseDTO.success = true;
                    aPIResponseDTO.message = "Subcontractor deleted successfully.";
                }
                else
                {
        ...
        public async Task<APIResponseDTO> ActiveInactive(int id, ExtractTokenDto token)
        {
            ...
            try
            {
                var data = await _context.Subcontractor.Where(x => x.id == id).FirstOrDefaultAsync();
                ...
                if (data != null)
                {
                    ...
                    _context.Subcontractor.Update(data);
                    await _context.SaveChangesAsync();

                    aPIResponseDTO.success = true;
                    aPIResponseDTO.message = "Subcontractor " + (data.active_flag == true ? "Activated" : "Inactivated") + " successfully.";
                }
                else
                {
