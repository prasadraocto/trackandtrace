CREATE PROCEDURE [dbo].[get_subcontractor_list]
    @company_id INT,
    @page INT = 1,
    @page_size INT = 10,
...
BEGIN
    ...
    FROM 
        subcontractor a
        INNER JOIN labour_type b ON a.labour_type_id = b.id AND b.active_flag = 1 AND b.delete_flag = 0
    WHERE 
    ...
    SET @SQL = N'SELECT 
        a.id, a.code, a.name, b.id as labour_type_id, b.name as labour_type_name, a.active_flag
    FROM 
        subcontractor a
        INNER JOIN labour_type b ON a.labour_type_id = b.id AND b.active_flag = 1 AND b.delete_flag = 0
    WHERE 
