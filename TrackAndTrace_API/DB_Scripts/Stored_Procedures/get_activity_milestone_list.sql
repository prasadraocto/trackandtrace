CREATE PROCEDURE [dbo].[get_activity_milestone_list]
    @project_id INT,
    @company_id INT,
    @page INT = 1,
    @page_size INT = 10,
...
BEGIN
    ...
    SELECT 
        @total_count = COUNT(*)
    FROM 
        activity_milestone a
    WHERE 
        a.project_id = @project_id AND a.company_id = @company_id AND a.delete_flag = 0 AND
        (
            @search_query IS NULL OR 
            (a.code LIKE '%' + @search_query + '%') OR
            (a.name LIKE '%' + @search_query + '%') OR
            (a.budget_cost LIKE '%' + @search_query + '%')
        );
    ...
    SET @SQL = N'SELECT 
        a.id, a.code, a.name, a.start_date, a.end_date, a.budget_cost, a.active_flag
    FROM 
        activity_milestone a
    WHERE 
        a.project_id = @project_id AND a.company_id = @company_id AND a.delete_flag = 0 AND
